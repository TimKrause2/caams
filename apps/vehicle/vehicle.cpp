...
int main(int argc, char **argv){
    ...
    if(!gc && !js){
        std::cout << "No game controllers or joysticks found. Exiting." << std::endl;
		return 1;
	}
	...
	while(running){
        ...
        if(gc){
            ...
            short js_x = SDL_JoystickGetAxis(js, 0);
            short js_y = SDL_JoystickGetAxis(js, 1);
            short js_z = SDL_JoystickGetAxis(js, 2);
            short js_t = SDL_JoystickGetAxis(js, 3);
            ...
            if(!js_zero){
                ...
                double js_xd = (double)(js_x-js_x0)/32768.0;
                double js_yd = (double)(js_y-js_y0)/32768.0;
                double js_zd = (double)(js_z-js_z0)/32768.0;
                double js_td = (double)(js_t-js_t0)/32768.0;

                if(SDL_JoystickGetButton(js, 0)){
                    localForce->f = Eigen::Vector3d(
                                0.0,
                                0.0,
                                js_td*FORCE_MAX);
                }else{
